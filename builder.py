...
# Convert list of lines to string for easier extraction
content = "".join(lines)

# 1. EXTRACT BREADCRUMBS
breadcrumbs = content[content.find('<Breadcrumbs'):content.find('</Breadcrumbs>') + 14]

# 2. EXTRACT TOP FILTER BAR
# The filter bar was at the bottom in the backup file
filter_start = content.find('{/* FILTER BAR */}')
filter_end = content.find('</Container>', filter_start) + 12
old_filter_bar = content[filter_start:filter_end]
...
# 3. EXTRACT HEADING & BADGES
heading_start = content.find('<h1 className="text-[1.5rem] font-bold tracking-wide">')
heading_end = content.find('</div>', content.find('<div className="flex justify-end gap-2 mt-2">')) + 6
heading_and_badges = content[heading_start:heading_end].replace('justify-end', 'justify-start')

# 4. EXTRACT IMAGE CAROUSEL (without the video block)
image_carousel_start = content.find('{/* Main Carousel (Images Only) */}')
image_carousel_end = content.find('{/* Conditional Video Block', image_carousel_start)
image_carousel = content[image_carousel_start:image_carousel_end].strip()

# 5. EXTRACT VIDEO BLOCK
video_start = content.find('{selectedGradeVideos.length > 0 && currentVideoUrl && (')
# Find the exact closing brace for the video block
video_end = content.find(')}', content.find('{/* Video Thumbnails Carousel')) + 2
# Add one more `)}` because of the `selectedGradeVideos.length > 0` check
video_end = content.find(')}', video_end) + 2
video_block = content[video_start:video_end].strip()

# 6. EXTRACT EXTRA DESCRIPTIONS
extra_desc_start = content.find('<EmeraldDetails')
extra_desc_end = content.find(') : null}', extra_desc_start) + 9
extra_descriptions = content[extra_desc_start:extra_desc_end].strip()

# 7. EXTRACT TEXT BLOCK
text_start = content.find('<p className="text-gray-700 leading-relaxed mt-4">')
text_end = content.find('</div>', content.find('Learn More About {data?.title}')) + 6
# Need to include the wrapping div of Buttons
text_end = content.find('</div>', text_end) + 6
text_block = content[text_start:text_end].strip()

# 8. EXTRACT EVERYTHING BEFORE GRID AND AFTER FILTER BAR
pre_grid = content[:content.find('<Grid>')]
# Actually, pre_grid ends right after breadcrumbs. Let's be exact.
pre_grid_end = content.find('</Breadcrumbs>') + 14
pre_grid = content[:pre_grid_end]

post_table_start = content.find('<CategoryTable')
table_end = content.find('/>', post_table_start) + 2
category_table = content[post_table_start:table_end]
