...
export const TopFilters = ({
  ...
  const hasActiveFilters = selectedGems.length > 0 || selectedShapes.length > 0 || weightRange.min !== "" || weightRange.max !== "" || lengthRange.min !== "" || lengthRange.max !== "" || widthRange.min !== "" || widthRange.max !== "" || singlePair !== "" || selectedSapphireColors.length > 0;
    ...
    <div className="w-full bg-white shadow-sm border border-gray-100 rounded-lg mb-10 relative z-10">
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        ...
        <div className="flex flex-col gap-2 min-w-[150px]">
           <Select
              label={<span className="text-sm font-bold text-[#0b182d] uppercase tracking-wide hidden">Single/Pair</span>}
              placeholder="Single/Pair"
              data={[{label: "Single", value: "Single"}, {label: "Pair", value: "Pair"}]}
              value={singlePair}
              onChange={(v) => setSinglePair(v || "")}
              clearable
              styles={{ input: { fontWeight: 600, color: '#0b182d', borderRadius: 20 } }}
              className="mt-6"
           />
        </div>
      </div>
      ...
      {hasActiveFilters && (
        <div className="mt-8 flex items-center gap-4 flex-wrap">
          ...
          {singlePair && (
             <div className="flex items-center gap-2 bg-gray-100 px-3 py-1 rounded-full text-xs font-semibold text-gray-800">
               {singlePair}
               <IconX size={14} className="cursor-pointer" onClick={() => setSinglePair("")} />
             </div>
          )}
